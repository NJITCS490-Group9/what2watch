...
const genreCardData = {
  ...
  Horror:
    "https://i.pinimg.com/originals/d8/06/08/d806085699179c5fc12ab69d91830f34.gif",
  Romance:
    "https://image.freepik.com/free-photo/couple-silhouettes-beach-sunset_106150-110.jpg",
};
...
function VotingScreen(props) {
  ...
  const voteSubmit = () => {
    alert(
      "You have submitted your vote! Please wait for the results to be calculated."
    );
  ...
  for (let i = 0; i < genres.length; i++) {
    genre_cards.push(
      <GenreCard name={genres[i]} voteSelect={voteSelect} key={i} />
    );
  }
  ...
  if (selectedGenre.length != 0) {
    ...
    return (
      <Results name={name} selectedGenre={selectedGenre} socket={socket} />
    );
  }
  ...
  return (
    ...
    <div class="container-fluid vote">
      ...
      <div class="card-columns">{genre_cards}</div>
    </div>
  );
}
    ...
    <div class="col-ld-5">
      <div class="card vote_card">
        ...
        <div class="card-body vote_card_body text-center">
          <img
            class="card-img-top genre_img"
            src={genreCardData[props.name]}
            alt={props.name}
          />
          <button
            type="button"
            className="genre_select_btn"
            value={props.name}
            onClick={props.voteSelect}
          >
            Select
          </button>
        </div>
